...
export const Breadcrumbs = React.forwardRef(function Breadcrumbs(
    ...
    const navProps = {
        ...
        'aria-label': props['aria-label'] || i18n('breadcrumbs'),
        'data-qa': props.qa,
    };

    const innerRef = React.useRef<HTMLElement>(null);
    const containerRef = useForkRef(ref, innerRef);
    ...
    const breadcrumbItems = contents.map((child, index) => {
        ...
        return (
            <li key={index} className={b('list-item', {calculating: !calculated})}>
                <BreadcrumbItem
                    {...child.props}
                    key={key}
                    isCurrent={isCurrent}
    ...
    return (
        <nav
            ...
            style={props.style}
        >
            <ol ref={listRef} className={b('list')}>
                {breadcrumbItems}
            </ol>
        </nav>
    );
